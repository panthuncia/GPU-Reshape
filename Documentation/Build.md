...
For a one-shot default Windows build, invoke `Build.bat`. It configures the Visual Studio solution, builds the full `Debug|x64` tree, and then runs the external UIX `.csproj` builds needed to restore `GPUReshape.exe` and related UIX outputs.
