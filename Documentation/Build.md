...
The Visual Studio solution generation helpers auto-detect the newest supported installed generator. On machines with Visual Studio 2026 installed, they will select `Visual Studio 18 2026`; otherwise they fall back to `Visual Studio 17 2022`.

For a one-shot default Windows build, invoke `Build.bat`. It configures the Visual Studio solution and builds the full `Debug|x64` tree. The generated Visual Studio solution now includes the UIX `.csproj` build pass, so building from the IDE or from `Build.bat` produces `GPUReshape.exe` and the related UIX outputs in one step.
